...
import { cx } from '@/helpers/chartUtils'
import {
...
const ChartTooltip = ({
	...
	if (active && payload && payload.length) {
		return (
			...
			>
				<div className={cx('space-y-1 px-3 py-2')}>
							...
							<div className="flex items-center space-x-2">
								...
								<p
									className={cx(
										...
										// text color
										'text-paragraph font-medium dark:text-gray-300'
									)}
								>
									{category}
								</p>
...
const DonutChart = React.forwardRef<HTMLDivElement, DonutChartProps>(
	(
		...
		return (
			<div
				ref={forwardedRef}
				className={cx('h-52 w-52', className)}
			...
			>
				<ResponsiveContainer className="size-full">
					...
					>
						{showLabel && isDonut && (
							<text
								className="fill-secondary GilroyBold text-2xl dark:fill-gray-300"
								x="50%"
								y="50%"
								textAnchor="middle"
						...
						<Pie
							className={cx(
								'stroke-white stroke-[5px] dark:stroke-gray-950 [&_.recharts-pie-sector]:outline-none',
								!isDonut && 'stroke-[5px]',
								onValueChange
									? 'cursor-pointer'
									: 'cursor-default'
