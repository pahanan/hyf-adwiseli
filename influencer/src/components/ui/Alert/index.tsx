...
const defaultRootClasses =
	'antialiased flex text-sm leading-6 bg-gray-50 items-start'
			...
			>
				<div
					className={cn(
						...
						variant === 'inline' &&
							'px-2 sm:flex-row sm:items-center sm:gap-2',
				...
				>
					...
					>
						{title && (
							<AlertTitle className="font-semibold" color={color}>
								{title}
							</AlertTitle>
						)}
						{children && (
				...
				{closable && (
					<AlertCloseButton
						className={cn(variant === 'inline' && 'pr-1')}
						onClick={handleClose}
					/>
				)}
			</AlertRoot>
...
>(({ className, children, ...otherProps }, ref) => {
	return (
		<div
			ref={ref}
			className={cn(defaultRootClasses, className)}
			role="alert"
			{...otherProps}
		>
			{children}
