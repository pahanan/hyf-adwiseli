				...
				>
					{({ getRootProps, getInputProps }) => (
						<div
							{...getRootProps()}
							className={cn(
								'rounded-xl bg-neutral-100 border border-dashed border-gray-600/10 text-sm hover:bg-neutral-50 cursor-pointer transition-all ease-in-out delay-50',
								step == 'crop' &&
									'opacity-50 hover:bg-neutral-200 cursor-not-allowed',
								className
							)}
						>
	...
	if (step == 'preview') {
		return (
			...
			>
				...
				/>
				<button
					className="hidden drop-shadow-lg group-hover:flex absolute rounded-full bg-red-500 p-2 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
					onClick={onImageRemove}
				>
					<Trash className="text-white" size={16} strokeWidth={2.5} />
				</button>
			</div>
		)
	}
